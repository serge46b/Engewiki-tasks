...
def update(dt):
    ...
    nc_filtred_img = filtered_img.copy()
    ...
    counter_img = copy.deepcopy(img)

    cv2.drawContours(counter_img, contours, -1, (255, 0, 0), 2)
    cv2.imshow("contours", counter_img)

    bound_box = []
    bound_box_img = copy.deepcopy(img)

    for i in contours:
        bound_box.append(cv2.boundingRect(i))
    bound_box_allowed = []
    for i in bound_box:
        delta_x = i[2] * 0.9
        delta_y = i[3] * 0.9

        if abs(i[2] - i[3]) > delta_x or abs(i[2] - i[3]) > delta_y or i[2] > 70 or i[2] < 25 or i[3] > 70 or i[3] < 25:
            #bound_box_allowed.append(i)
            pass
        else:
            bound_box_allowed.append(i)
            #print(i[2], i[3])
    #print(bound_box)
    filtered_img_allowed = []
    for i in bound_box_allowed:
        cv2.rectangle(bound_box_img, (i[0], i[1]), (i[0] + i[2], i[1] + i[3]), (0, 255, 0), 2)
        filtered_img_allowed.append(nc_filtred_img.copy()[i[1]:i[1] + i[3], i[0]:i[0] + i[2]])
    cv2.imshow("bound boxes", bound_box_img)
    # for i in range(len(filtered_img_allowed)):
    #     cv2.imshow(str(i), filtered_img_allowed[i])



    """stop_template = cv2.imread('stop_template.png', 0)
    w, h = stop_template.shape[::-1]

    # methods = ['cv2.TM_CCOEFF', 'cv2.TM_CCOEFF_NORMED', 'cv2.TM_CCORR',
    #           'cv2.TM_CCORR_NORMED', 'cv2.TM_SQDIFF', 'cv2.TM_SQDIFF_NORMED']
    template_match_img = []
    for i in filtered_img_allowed:
        template_match_img.append(nc_filtred_img.copy())
        # Apply template Matching
        #res = cv2.matchTemplate(template_match_img[len(template_match_img) - 1], stop_template, cv2.TM_CCOEFF_NORMED)
        res = cv2.matchTemplate(filtered_img.copy(), stop_template, cv2.TM_CCORR_NORMED)
        loc = np.where(res >= 0.8)
        for pt in zip(*loc[::-1]):
            print(pt)
            cv2.rectangle(template_match_img[len(template_match_img) - 1], pt, (pt[0] + w, pt[1] + h), (255, 255, 255), 2)
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res)
        # If the method is TM_SQDIFF or TM_SQDIFF_NORMED, take minimum
        if methods[1] in [cv2.TM_SQDIFF, cv2.TM_SQDIFF_NORMED]:
            top_left = min_loc
        else:
            top_left = max_loc
        bottom_right = (top_left[0] + w, top_left[1] + h)
        cv2.rectangle(template_match_img[len(template_match_img) - 1], top_left, bottom_right, 255, 2)

    for i in range(len(template_match_img)):
        cv2.imshow("template_math" + str(i), template_match_img[i])
